...
double get_time() {
    ...
    return elapsed.count(); // en segundos
}
...
unsigned char *readBMP(char *file_name, char header[54], int *w, int *h)
{
	...
	// Si no lee 54 bytes es que la imagen de entrada es demasiado pequena
	if (n != 54)
		fprintf(stderr, "Entrada muy pequena (%d bytes)\n", n), exit(1);

	// Si los dos primeros bytes no corresponden con los caracteres BM no es un fichero BMP
	if (header[0] != 'B' || header[1] != 'M')
		fprintf(stderr, "No BMP\n"), exit(1);

	// El tamano de la imagen es el valor de la posicion 2 de la cabecera menos 54 bytes que ocupa esa cabecera
	int imagesize = *(int *)(header + 2) - 54;
	printf("Tamano archivo = %d\n", imagesize);

	// Si la imagen tiene tamano negativo o es superior a 48MB la imagen se rechaza
	if (imagesize <= 0 || imagesize > 0x3000000)
		fprintf(stderr, "Imagen muy grande: %d bytes\n", imagesize), exit(1);

	// Si la cabecera no tiene el tamano de 54 o el numero de bits por pixel es distinto de 24 la imagen se rechaza
