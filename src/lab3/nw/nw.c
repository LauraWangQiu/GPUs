...
void runTest(int argc, char **argv)
{
	...
	for (int i = 0; i < max_cols - 1; i++)
	{
		if (blosum)
			...
			input1[i] = set_amino_symbol(string1[i]);
	}
	for (int j = 0; j < max_rows - 1; j++)
	{
		if (blosum)
			...
			input2[j] = set_amino_symbol(string2[j]);
	}
...
#define TRACEBACK
	...
	for (int i = 0; i < max_rows; i++)
	{
		if (i < 1)
			...
			printf("%c ", get_amino_symbol(input2[i - 1]));
		for (int j = 0; j < max_cols; j++)
		{
			if (nw_matrix[i * max_cols + j] >= 0)
				if (nw_matrix[i * max_cols + j] < 10)
					...
					printf("  %i ", nw_matrix[i * max_cols + j]);
			else if (nw_matrix[i * max_cols + j] <= -10)
				printf(" %i ", nw_matrix[i * max_cols + j]);
			else
				printf("  %i ", nw_matrix[i * max_cols + j]);
		}
	...
	for (int i = 0; i < max_rows - 1; i++)
		printf("%c", get_amino_symbol(input2[i]));
	printf("\n");
